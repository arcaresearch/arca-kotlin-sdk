...
class WebSocketManagerTest {
    ...
    @Test
    fun streamResyncFiresGapHandlerWithFloorOfOne() = runBlocking {
        val manager = makeManager()
        val gaps = Collections.synchronizedList(mutableListOf<Int>())
        manager.onGap { gaps.add(it) }

        manager.injectMessage("""{"type":"mids.updated","mids":{"hl:0:BTC":"1"},"deliverySeq":1}""")
        manager.injectMessage("""{"type":"stream.resync","reason":"event_loss","deliverySeq":2}""")

        // The server knows events were lost, not how many: the count is a
        // floor of 1.
        assertEquals(listOf(1), gaps.toList())
    }

    @Test
    fun streamResyncKeepsSequenceContiguous() = runBlocking {
        val manager = makeManager()
        val gaps = Collections.synchronizedList(mutableListOf<Int>())
        manager.onGap { gaps.add(it) }
        ...
        manager.injectMessage("""{"type":"mids.updated","mids":{"hl:0:BTC":"2"},"deliverySeq":3}""")

        assertEquals(listOf(1), gaps.toList())
    }

    @Test
    fun streamResyncIsNotEmittedToEventFlows() = runBlocking {
        val manager = makeManager()
        val received = collect(manager.events) {
            manager.injectMessage("""{"type":"stream.resync","reason":"event_loss","deliverySeq":1}""")
            manager.injectMessage("""{"type":"object.updated","path":"/after","deliverySeq":2}""")
        }
        // The real event arrives; the control message does not.
        assertEquals(listOf("object.updated"), received.map { it.type })
    }

    @Test
    fun streamResyncAlsoReportsSequenceGapWhenMarkerRevealsOne() = runBlocking {
        val manager = makeManager()
        val gaps = Collections.synchronizedList(mutableListOf<Int>())
        manager.onGap { gaps.add(it) }
        ...
        manager.injectMessage("""{"type":"stream.resync","reason":"event_loss","deliverySeq":5}""")

        assertEquals(listOf(3, 1), gaps.toList())
    }
